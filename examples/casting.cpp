/**
 * @file examples/casting.cpp
 * @brief Example program showing how to use ray/sphere transformations to display a circle.
...
int main(int argc, char* argv[], char* envp[])
{
    ...
    //set_transform(sphere, scaling(1, 0.5, 1));

    // shrink it along the x axis
    //set_transform(sphere, scaling(0.5, 1, 1));

    // shrink it and rotate it
    //set_transform(sphere, rotation_z(M_PI_4) * scaling(0.5, 1, 1));

    // shrink it and skew it
    // set_transform(sphere, shearing(1, 0, 0, 0, 0, 0) * scaling(0.5, 1, 1));
